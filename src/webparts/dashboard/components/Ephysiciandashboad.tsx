  ...
  public async GetLetterAuthorizationListItems(ViewMode) {
    ...
    if (ViewMode == "UnitHR") {
      for (var i = 0; i < HrUnitNames.length; i++) {
        await newweb.lists.getByTitle("E Physician Profile Transaction").items.select(
          "ID",
          "AuthorityName",
          "LicenseNumber",
          "Status",
        ...
        await newweb.lists.getByTitle('Onboarding Transaction Master').items
          ...
          .filter(`BusinessUnit eq '${HrUnitNames[i]}' and Title eq 'EPHYSICAN PROFILE' and Status ne 'Completed'`)
          .get().then((items) => {
            ...
            if (items.length != 0) {
              for (var i = 0; i < items.length; i++) {
                MasterGlobArray.push({
                  ...
                  AuthorityName: items[i].AssignedTo.Title,
                  Created: "-",
      ...
      await newweb.lists
        ...
        .then((response) => {
          if (response.length != 0) {
            ...
            //(resholder);
            setTimeout(() => {
              ($("#exampleephysician") as any).DataTable();
            }, 1500);
          } else {
            setTimeout(() => {
              ($("#exampleephysician") as any).DataTable();
    ...
    //       "Created"
    //     )
    //     .filter(`Author/Id eq ${curentloaid}`)
    //     .expand("Author")
    //     .get()
    //     .then((response) => {
    //       if (response.length != 0) {
    ...
    const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (
      ...
      return (
        <tr>
          ...
          <td>
            <a
              href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?ephysicianItemID=${item.ID}&EphysicianMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
            >
              <span>
                <img
            ...
            {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
              <a id={`${item.ONBSessionID}`}
                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?ephysicianItemID=${item.ID}&EphysicianMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
              >
                <span>
                  <img
      ...
      <div >
        ...
        <div className="db-multiple-table mb-20 sec">
          <div className="dashboard-title clearfix">
            ...
            <div>
              <table id="exampleephysician" className="display table-striped list_of_table">
                <thead>
                  <tr>
