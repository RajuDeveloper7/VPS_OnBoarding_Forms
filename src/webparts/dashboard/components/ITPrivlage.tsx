    ...
    if (ViewMode == "UnitHR") {
      for (var i = 0; i < HrUnitNames.length; i++) {
        ...
        await newweb.lists.getByTitle('Onboarding Transaction Master').items
          ...
          .filter(`BusinessUnit eq '${HrUnitNames[i]}' and Title eq 'HR IT PRIVILEGE FORM' and Status ne 'Completed'`)
          .get().then((items) => {
    ...
    const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (
      ...
      return (
        <tr>
          ...
          <td>
            <a
              href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?LoaItemID=${item.ID}&LoaMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
            >
              <span>
                <img
            ...
            {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
              <a id={`${item.ONBSessionID}`}
                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?LoaItemID=${item.ID}&LoaMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
              >
                <span>
                  <img
            ...
            {/* <a href="#" onClick={() => handler.DeleteItem(item.ID, "HR IT Privilege Transaction")}><span><img src="https://vpshealth.sharepoint.com/sites/BurjeelHoldings/Site%20Asset/Remo%20Portal%20Assets/img/delete.svg" className="delete_btn" alt="image"/></span></a> */}
          </td>
      ...
      <div >
        ...
        <div className="db-multiple-table mb-20 sec">
          <div className="dashboard-title clearfix">
            ...
            <div>
              <table id="exampleHRit" className="display table-striped list_of_table">
                <thead>
                  <tr>
