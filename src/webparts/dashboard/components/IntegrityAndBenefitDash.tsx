...
export interface IIntegrityDashboardState {
    Items: any[];
...
export default class IntegrityDashboard extends React.Component<IDashboardProps, IIntegrityDashboardState, {}> {
    public constructor(props: IDashboardProps, state: IIntegrityDashboardState) {
        super(props);
        this.state = {
            Items: [],
    ...
    public componentDidMount() {
        ...
        this.GetCurrentUserIDIntegrity();
    }

    public GetCurrentUserIDIntegrity() {
    ...
    public async GetcurrentUserViewLevels(personalid) {
        ...
        if (this.state.IsHeadHR == true) {
            this.GetIntegrityListItems("HRHead");
        } else if (this.state.IsUnitHR == true && this.state.IsGroupHR == false) {
            this.GetIntegrityListItems("UnitHR");
        } else if (this.state.IsUnitHR == false && this.state.IsGroupHR == true) {
            this.GetIntegrityListItems("GroupHR");
        } else if (this.state.IsUnitHR == true && this.state.IsGroupHR == true) {
            this.GetIntegrityListItems("GroupHR-UnitHR");
        }
    }



    public async GetIntegrityListItems(ViewMode) {
            ...
            await newweb.lists.getByTitle("Integrity And Benefit Disclosure Transaction")
                ...
                .then((response) => {
                    if (response.length != 0) {
                        ...
                        //(resholder);
                        setTimeout(() => {
                            ($("#example-integrity") as any).DataTable();
                        }, 1500);
                    } else {
                        setTimeout(() => {
                            ($("#example-integrity") as any).DataTable();
        ...
        const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (item, key) {
            ...
            return (
                <tr>
                    ...
                    <td>
                        <a
                            href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?IntegrityItemID=${item.ID}&IntegrityMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
                        >
                            <span>
                                <img
                        ...
                        {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
                            <a id={`${item.ONBSessionID}`}
                                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?IntegrityItemID=${item.ID}&IntegrityMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
                            >
                                <span>
                                    <img
            ...
            <div >
                ...
                <div className="db-multiple-table mb-20 sec">
                    <div className="dashboard-title clearfix">
                        ...
                        <div>
                            <table id="example-integrity" className="display table-striped list_of_table">
                                <thead>
                                    <tr>
