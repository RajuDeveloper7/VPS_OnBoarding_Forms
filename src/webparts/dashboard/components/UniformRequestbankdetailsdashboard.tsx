...
var currentuniformid: number = null;
  ...
  public async GetLetterAuthorizationListItems(ViewMode) {
    ...
    if (ViewMode == "UnitHR") {
      for (var i = 0; i < HrUnitNames.length; i++) {
        await newweb.lists.getByTitle("EmployeeBankDetailsMaster").items.select(
          "ID",
          "Date",
          "NameofEmployee",
          "EmployeeId",
          ...
          .filter(`BussinessUnit eq '${HrUnitNames[i]}'`)
          .expand("Author")
          .get()
          .then((response) => {
        ...
        await newweb.lists.getByTitle('Onboarding Transaction Master').items
          ...
          .filter(`BusinessUnit eq '${HrUnitNames[i]}' and Title eq 'UNIFORM REQUEST BANK DETAILS' and Status ne 'Completed'`)
          .get().then((items) => {
            ...
            if (items.length != 0) {
              for (var i = 0; i < items.length; i++) {
                MasterGlobArray.push({
                  ...
                  NameofEmployee: items[i].AssignedTo.Title,
                  Created: "-",
                  BussinessUnit: items[i].BusinessUnit,
                  Author: {
                    Title: "-"
                  }
                });
      ...
      await newweb.lists
        ...
        .then((response) => {
          if (response.length != 0) {
            ...
            for (var i = 0; i < response.length; i++) {
              if (tempstring.indexOf(response[i].BussinessUnit) != -1) {
                //Burjeelholdings
            ...
            //(resholder);
            setTimeout(() => {
              ($("#exampleuniform") as any).DataTable();
            }, 1500);
          } else {
            setTimeout(() => {
              ($("#exampleuniform") as any).DataTable();
    ...
    //       "Created"
    //     )
    //     .filter(`Author/Id eq ${currentuniformid}`)
    //     .expand("Author")
    //     .get()
    //     .then((response) => {
    //       if (response.length != 0) {
    ...
    const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (
      ...
      return (
        <tr>
          ...
          <td>
            <a
              href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?BankdetailsItemID=${item.ID}&URBDMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`}
            ...
            {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
              <a id={`${item.ONBSessionID}`}
                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?BankdetailsItemID=${item.ID}&URBDMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`}
                data-interception="off" target="_blank"
              >
                <span>
                  <img
      ...
      <div >
        ...
        <div className="db-multiple-table mb-20 sec">
          <div className="dashboard-title clearfix">
            ...
            <div>
              <table id="exampleuniform" className="display table-striped list_of_table">
                <thead>
                  <tr>
