...
export interface ICoiDashboardState {
  Items: any[];
...
var curentcoiid: number = null;
export default class CoiDashboard extends React.Component<
  IDashboardProps,
  ICoiDashboardState,
  {}
> {
  ...
  public componentDidMount() {
    ...
    this.GetCurrentUserIDcoi();

  }
  public GetCurrentUserIDcoi() {
    ...
    let curruser = newweb.currentUser.get().then(function (res) {
      ...
      setTimeout(() => {
        reacthandler.GetCOIListItems("Creator");
      }, 5000);
    })
  }
  ...
  //         var userid=resultData.d.Id;
  //         currentcoiid=resultData.d.Id;
  //         reacthandler.GetcurrentUserViewLevels(userid)
  //         reacthandler.GetCOIListItems("Creator");
  //       },
  ...
  public async GetCOIListItems(ViewMode) {
    ...
    if (ViewMode == "UnitHR") {
      for (var i = 0; i < HrUnitNames.length; i++) {
        ...
        await newweb.lists.getByTitle('Onboarding Transaction Master').items
          ...
          .filter(`BusinessUnit eq '${HrUnitNames[i]}' and Title eq 'CONFLICT OF INTEREST' and Status ne 'Completed'`)
          .get().then((items) => {
    ...
    //       "Created"
    //     )
    //     .filter(`Author/Id eq ${curentcoiid}`)
    //     .expand("Author")
    //     .get()
    //     .then((response) => {
    //       if (response.length != 0) {
    ...
    const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (
      ...
      return (
        <tr>
          ...
          <td>
            <a
              href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?CoiItemID=${item.ID}&CoiMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
            >
              <span>
                <img
            ...
            {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
              <a id={`${item.ONBSessionID}`}
                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?CoiItemID=${item.ID}&CoiMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
              >
                <span>
                  <img
          ...
          {/* <td>{moment(item.WitnessDate).format("MM/DD/YYYY")}</td> */}
          {/* <td>{item.ContractTransactionHistory}</td> */}
        ...
        <div className="db-multiple-table mb-20 sec">
          <div className="dashboard-title clearfix">
            ...
            <div className="table-scroll">
              <table id="examplecoii" className="display table-striped list_of_table">
                <thead>
                  <tr>
                    ...
                    {/* <th>Witness Name</th> */}
                    {/* <th>Witness Date</th> */}
                    {/* <th>Contract Transaction History</th> */}
                ...
                {/* <p id="coi-nodata">No available content to show</p> */}
