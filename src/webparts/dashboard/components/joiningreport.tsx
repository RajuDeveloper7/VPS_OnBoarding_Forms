...
// );
// SPComponentLoader.loadCss(
//     "https://vpshealth.sharepoint.com/sites/BurjeelHoldings/Site%20Asset/Remo%20Portal%20Assets/css/form%20css/style.css?v=9.6"
// );
export interface IJoiningReportDashboardState {
    Items: any[];
...
var curentJrid: number = null;
export default class JoiningReportdashboard extends React.Component<
    IDashboardProps,
    IJoiningReportDashboardState,
    {}
> {
    public constructor(
        props: IDashboardProps,
        state: IJoiningReportDashboardState
    ) {
        super(props);
        this.state = {
            Items: [],
    ...
    public GetCurrentUserIDJr() {
        var reacthandler = this;
        let curruser = newweb.currentUser.get().then(function (res) {
            console.log(res);
            var Jrcurrentuserid = res.Id;
            curentJrid = res.Id;
            reacthandler.GetcurrentUserViewLevelsJr(Jrcurrentuserid);
            setTimeout(() => {
                reacthandler.GetJoiningReportListItems("Creator");
            }, 5000);
        })
    }
    ...
    //   var reacthandler = this;
    //   $.ajax({
    //     url: `${reacthandler.props.siteurl}/_api/web/currentuser`,//?$select=Id
    //     type: "GET",
    //     headers: { Accept: "application/json; odata=verbose;" },
    //     success: function (resultData) {  
    //       console.log (resultData);
    //       var useridcloa=resultData.d.Id;
    //       curentid = resultData.d.Id;
    //       reacthandler.GetLetterAuthorizationListItems("Creator");
    //       reacthandler.GetcurrentUserViewLevelsloa(useridcloa);
    //     },
    //     error: function (jqXHR, textStatus, errorThrown) {
    //     },
    //   });
    // }
    public async GetcurrentUserViewLevelsJr(personalid) {
        //Unit hr
        await newweb.lists
        ...
        if (this.state.IsHeadHR == true) {
            this.GetJoiningReportListItems("HRHead");
        } else if (this.state.IsUnitHR == true && this.state.IsGroupHR == false) {
            this.GetJoiningReportListItems("UnitHR");
        } else if (this.state.IsUnitHR == false && this.state.IsGroupHR == true) {
            this.GetJoiningReportListItems("GroupHR");
        } else if (this.state.IsUnitHR == true && this.state.IsGroupHR == true) {
            this.GetJoiningReportListItems("GroupHR-UnitHR");
        }
    }
    public async GetJoiningReportListItems(ViewMode) {
        ...
        if (ViewMode == "UnitHR") {
            for (var i = 0; i < HrUnitNames.length; i++) {
                ...
                await newweb.lists.getByTitle('Onboarding Transaction Master').items
                    ...
                    .filter(`BusinessUnit eq '${HrUnitNames[i]}' and Title eq 'JOINING REPORT' and Status ne 'Completed'`)
                    .get().then((items) => {
            ...
            await newweb.lists
                ...
                .then((response) => {
                    if (response.length != 0) {
                        ...
                        //(resholder);
                        setTimeout(() => {
                            ($("#examplejoiningreport") as any).DataTable();
                        }, 1500);
                    } else {
                        setTimeout(() => {
                            ($("#examplejoiningreport") as any).DataTable();
        ...
        //             "Created"
        //         )
        //         .filter(`Author/Id eq ${curentJrid}`)
        //         .expand("Author")
        //         .get()
        //         .then((response) => {
        //             if (response.length != 0) {
    ...
    public render(): React.ReactElement<IDashboardProps> {
        ...
        const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (
            ...
            return (
                <tr>
                    <td style={{ width: "50px" }}>{item.ONBSessionID}</td>
                    <td>
                        <a
                            href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?JoiningItemId=${item.ID}&JoiningreportMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank">
                            <span>
                                <img
                        ...
                        {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
                            <a id={`${item.ONBSessionID}`}
                                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?JoiningItemId=${item.ID}&JoiningreportMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
                            >
                                <span>
                                    <img
                    ...
                    <td>{item.Name}</td>
                    {/* <td>{moment(item.DateofJoining).format("MM/DD/YYYY")}</td>
                ...
                >
                  Add
                        ...
                        <div className="table-scroll">
                            <div>
                                <table id="examplejoiningreport" className="display table-striped list_of_table">
                                    <thead>
                                        <tr>
                                            <th style={{ width: "50px" }}>Onboarding Request ID</th>
