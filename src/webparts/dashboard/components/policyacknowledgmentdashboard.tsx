...
var curentpolicyids: number = null;
...
> {
  ...
  public async GetLetterAuthorizationListItems(ViewMode) {
    ...
    if (ViewMode == "UnitHR") {
      for (var i = 0; i < HrUnitNames.length; i++) {
        ...
        await newweb.lists.getByTitle('Onboarding Transaction Master').items
          ...
          .filter(`BusinessUnit eq '${HrUnitNames[i]}' and Title eq 'POLICY ACKNOWLEDGMENT AND DECLARATION' and Status ne 'Completed'`)
          .get().then((items) => {
            ...
            if (items.length != 0) {
              for (var i = 0; i < items.length; i++) {
                MasterGlobArray.push({
                  ...
                  EmployeePrintedName: items[i].AssignedTo.Title,
                  Created: "-",
      ...
      await newweb.lists
        ...
        .then((response) => {
          if (response.length != 0) {
            ...
            //(resholder);
            setTimeout(() => {
              ($("#examplepolicydash") as any).DataTable();
            }, 1500);
          } else {
            setTimeout(() => {
              ($("#examplepolicydash") as any).DataTable();
    ...
    //       "Created"
    //     )
    //     .filter(`Author/Id eq ${curentpolicyids}`)
    //     .expand("Author")
    //     .get()
    //     .then((response) => {
    //       if (response.length != 0) {
    ...
    const DynamicTableRows: JSX.Element[] = handler.state.Items.map(function (
      ...
      return (
        <tr>
          ...
          <td>
            <a
              href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?policyItemID=${item.ID}&PADCMode=View&mdeopn=View&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
            >
              <span>
                <img
            ...
            {handler.state.IsUnitHR == true && item.Id != "notfilled" && (
              <a id={`${item.ONBSessionID}`}
                href={`https://vpshealth.sharepoint.com/sites/BurjeelHoldings/HRFORM/SitePages/HrOnBoardingForm.aspx?policyItemID=${item.ID}&PADCMode=Edit&mdeopn=Edit&glblsessid=${item.ONBSessionID}&env=WebView`} data-interception="off" target="_blank"
              >
                <span>
                  <img
          ...
          <td>{item.EmployeePrintedName}</td>
      ...
      <div >
        ...
        <div className="db-multiple-table mb-20 sec">
          <div className="dashboard-title clearfix">
            <div className="table-wrapper-date">
              <div className="table-search">
                <h3 className="contact-pg-title">
                  ...
                  Policy Aknowledgement & Declarations (Clinicians/Nursing/Allied Health Staff Specific)
                </h3>
              </div>
            ...
            <div>
              <table id="examplepolicydash" className="display table-striped list_of_table">
                <thead>
                  <tr>
